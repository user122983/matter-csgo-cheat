...
bool pe::setup( ) {
	...
	for ( _LIST_ENTRY* it = list->Flink; it != list; it = it->Flink ) {
		...
		std::wstring wide_name = std::wstring( base_dll_name->Buffer, base_dll_name->Length >> 1 );
		std::string name = std::string( wide_name.begin( ), wide_name.end( ) );
...
std::size_t pe::export_fn( std::size_t base, std::size_t fn_hash, bool in_memory ) {
	...
	for ( std::uint32_t i = 0; i < exports->NumberOfFunctions; i++ ) {
		...
		const char* function_name = reinterpret_cast< const char* >( base + rva_2_offset( names[ i ], nt_headers, in_memory ) );
