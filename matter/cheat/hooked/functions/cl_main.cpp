...
struct cl_msg_move {
	...
	~cl_msg_move( ) {
		...
		const auto function = m_signatures.m_cl_msg_move_deconstructor.as< void( __thiscall* )( void* ) >( );
	...
	auto set_data( unsigned char* data, const int num_bytes_written ) {
		...
		const auto function = m_signatures.m_set_data.as< void* ( __thiscall* )( void*, unsigned char*, std::size_t ) >( );
