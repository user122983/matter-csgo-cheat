...
void tab_panel::geometry( ) {
	...
	for ( std::size_t i = 0; i < m_tabs.size( ); i++ ) {
		...
		if ( m_selected_entry == i ) {
			...
			m_render.draw_text( m_render.m_fonts.matter_icons, m_widget_area.m_x + 50, m_widget_area.m_y + m_size.m_height / 2, m_tabs[ i ].first, color( 255, 255, 255, 50 ), x_centre | y_centre );
			m_render.draw_text( m_render.m_fonts.verdana, m_widget_area.m_x + 80, m_widget_area.m_y + m_size.m_height / 2, m_tabs[ i ].second, color( 255, 255, 255, 50 ), y_centre);
